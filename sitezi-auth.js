/* =========================================================
   SITEZI — AUTENTICAÇÃO v1.0
   Cadastro/Login real com Supabase Auth.
   Publicação exige conta autenticada.
   ========================================================= */
...
(async () => {
  ...
  style.textContent = `
    ...
    .sitezi-auth-close{
      border:0;background:transparent;color:#9fb0c8;
      font-size:28px;line-height:1;cursor:pointer
    }
    .sitezi-auth-copy{
      color:#94a3b8;margin:0 0 18px;line-height:1.45
    }
    .sitezi-auth-tabs{
    ...
    .sitezi-auth-tab{
      ...
      color:#91a0b7;padding:11px;font-weight:800;cursor:pointer
    }
    ...
    .sitezi-auth-form label{
      display:grid;gap:6px;font-size:13px;color:#aebbd0
    }
    ...
    .sitezi-auth-form input:focus{
      border-color:#2368ff;box-shadow:0 0 0 3px rgba(35,104,255,.14)
    }
    .sitezi-auth-submit{
    ...
    .sitezi-auth-message{
      min-height:22px;margin-top:12px;font-size:13px;
      line-height:1.4;color:#9fb0c8
    }
    ...
    @media(max-width:760px){
      .sitezi-auth-trigger{display:none}
  ...
  modal.innerHTML = `
    <div class="sitezi-auth-card" role="dialog" aria-modal="true" aria-labelledby="siteziAuthTitle">
      ...
      <p class="sitezi-auth-copy">
        Crie sua conta para salvar seu site e seguir para a publicação.
      </p>
      ...
      <div id="siteziAuthLoggedOut">
        ...
        <form id="siteziLoginForm" class="sitezi-auth-form">
          <label>
            E-mail
            <input id="siteziLoginEmail" type="email" autocomplete="email" required placeholder="voce@email.com">
          </label>
          <label>
            Senha
            <input id="siteziLoginPassword" type="password" autocomplete="current-password" required minlength="6" placeholder="Sua senha">
          </label>
        ...
        <form id="siteziSignupForm" class="sitezi-auth-form hidden">
          <label>
            Seu nome
            <input id="siteziSignupName" type="text" autocomplete="name" required maxlength="80" placeholder="Seu nome">
          </label>
          <label>
            E-mail
            <input id="siteziSignupEmail" type="email" autocomplete="email" required placeholder="voce@email.com">
          </label>
          <label>
            Senha
            <input id="siteziSignupPassword" type="password" autocomplete="new-password" required minlength="6" placeholder="Mínimo de 6 caracteres">
          </label>
      ...
      <div id="siteziAuthLoggedIn" class="sitezi-auth-userbox hidden">
        ...
        <button id="siteziAuthContinue" class="sitezi-auth-submit" type="button">
          Continuar para publicação →
        </button>
        <button id="siteziAuthLogout" class="sitezi-auth-logout" type="button">
          Sair da conta
        </button>
      </div>
  ...
  function updateUI() {
    ...
    const trigger = $("siteziAccountButton");

    if (currentUser) {
      loggedOut.classList.add("hidden");
      loggedIn.classList.remove("hidden");
      email.textContent = currentUser.email || "Conta conectada";
      if (trigger) trigger.textContent = "Minha conta";
    } else {
      loggedOut.classList.remove("hidden");
      loggedIn.classList.add("hidden");
      if (trigger) trigger.textContent = "Entrar";
    }
  }
  ...
  function draftPayload() {
    ...
    return {
      ...
      business_type:
        document.querySelector(".business.active")?.dataset.business || "Outro",
      template:
        document.querySelector(".template-card.active")?.dataset.template || "modern",
      status: "draft",
      configuration: {
        ...
        color:
          document.querySelector(".color.active")?.dataset.color || "#1578ff",
        logo_mode:
          document.querySelector("[data-logo-mode].active")?.dataset.logoMode || "text",
        image_mode:
          document.querySelector("[data-image-mode].active")?.dataset.imageMode || "none"
      },
      generated_content: {
        html: frame?.srcdoc || ""
      }
  ...
  async function runPendingPublish() {
    ...
    try {
      ...
      setMessage(
        "Sua conta entrou, mas não consegui salvar a prévia agora. Tente novamente.",
        "error"
      );
    } finally {
      if (continueBtn) {
        continueBtn.disabled = false;
        continueBtn.textContent = "Continuar para publicação →";
      }
    }
  }
  ...
  $("siteziSignupForm").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        setMessage(
          "Conta criada. Confira seu e-mail para confirmar o cadastro e depois entre na SITEZI.",
          "ok"
        );
      }
    } catch (error) {
  ...
  modal
    .querySelector(".sitezi-auth-close")
  ...
  document.documentElement.dataset.siteziAuth = "1.0";
  console.info("[SITEZI] Supabase Auth v1.0 carregado.");
})();
