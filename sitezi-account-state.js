...
(async () => {
  ...
  style.textContent=`
    ...
    #siteziTopCredits{margin-left:auto;margin-right:8px}.wizard-head #siteziWizardCredits{margin-left:auto;margin-right:10px}
    @media(max-width:850px){.topbar #siteziTopCredits.visible{display:flex!important;margin-left:auto;margin-right:4px;padding:6px 8px;gap:5px;min-height:32px;font-size:9.5px;max-width:190px;overflow:hidden}.topbar #siteziTopCredits .sitezi-credit-dot{display:none}.topbar #siteziTopCredits .sitezi-plan{max-width:72px;overflow:hidden;text-overflow:ellipsis}.wizard-head #siteziWizardCredits{margin-left:auto;margin-right:2px;padding:6px 8px;gap:6px;font-size:10px}.wizard-head #siteziWizardCredits .sitezi-plan{display:none}}
    @media(max-width:560px){.topbar #siteziTopCredits.visible{max-width:155px;font-size:9px;padding:5px 7px}.topbar #siteziTopCredits .sitezi-plan{max-width:58px}.topbar #siteziTopCredits .sitezi-credit-item{font-size:9px}}
  `;
  ...
  async function refresh(){
    ...
    const [bal,sub]=await Promise.all([
      ...
      client.from("subscriptions").select("plan,status,current_period_end").eq("user_id",currentUser.id).eq("status","active").gt("current_period_end",now).order("current_period_end",{ascending:false}).limit(1).maybeSingle()
    ]);
    const info={active:!!sub.data,plan:sub.data?.plan||"",images:bal.data?.image_credits??0,logos:bal.data?.logo_credits??0,resetAt:bal.data?.reset_at||sub.data?.current_period_end||null};
  ...
  document.documentElement.dataset.siteziAccountState="2.0";
})();
